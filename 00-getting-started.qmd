...
<!-- DK: Maybe don't need to set options to binary explicitly anymore, now that we have set_binary function, which we could just call. -->
...
This package provides infrastructure for doing tutorials, a common apprach for learning R.

Once you have **tutorial.helpers** installed, run these commands.
...
These functions set various R/RStudio options to convenient values.
...
Let's install two packages that is not available from CRAN: **r4ds.tutorials** and **all.primer.tutorials**. Copy and paste the following to the R Console:
...
This package is fairly simple and should install without any problems. Many other new packages will be installed as well. 
...
For each chapter of the textbook, there are one or more tutorials available in the **r4ds.tutorials** or **all.primer.tutorials** package. In order to access these tutorials, you should run `library(r4ds.tutorials)` or `library(all.primer.tutorials)` in the R Console.
...
### Go Read *R for Data Science (2e)* {.unnumbered}
...
You should have done the following:
  ...
  - Installed, from CRAN, the **remotes** and **tutorial.helpers** packages:
...
```
  ...
  - Installed, from Github, the **r4ds.tutorials**, **primer.data**, and **all.primer.tutorials** packages:
