...
Let's install two packages that are not available from CRAN: **r4ds.tutorials** and **primer.tutorials**. Copy and paste the following to the R Console:
...
<!-- DK: Will need to update this once we remove primer.data from the r4ds.tutorials package. But, even then, we will certainly need primer.data in the primer.tutorials package, so we will just be moving this discussion lower. -->
