...
When we take a look at both of our variables: age and party there are several possibilities that may exist. For example, what would occur if the party would change for each person and the definition change, how would that affect our model? What would happen if we decided to use a different family besides "gaussian?" What would happen if we changed the seed? Or refresh? All of these seemingly "minute" factors play a strong role in changing the outcome of our fitted model and how we are able to provide more accurate results with the help of our model. 

<!-- DK: How can we check that the DGM is ok? Answer: By creating some fake data sets, and making sure that our real data set "looks like" the fake ones. -->
