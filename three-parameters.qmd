...
<!-- DK: Always need to ask specify the moment in time to which the Preceptor Table refers. The original question will almost never specify this, although the normal answer is often "now" or "today." Ask this question as part of the what is the outcome question. DONE-->

<!-- Remove sampling & assignment mechanism -->

<!-- DK: In each case, make the "refined" question explicit right before creating the Preceptor Table. DONE-->

<!-- DK: In the Validity section, always discuss how we need to refine the question further in order to answer it, because variables do not come out of this air. -->

<!-- DK: Capitalize Population Table. DONE-->

<!-- DK: Discussions of representativeness might seem redundant. And, of course, we always mention the two potential problems: Is our data representative of the population and is our Preceptor Table representative of the population? Each of these points gets its own short paragraph. There are always two levels of concerns: first is generic, second only applies to the variables you need to answer the question. applied in age~party (needs to be applied elsewhere)-->

<!-- MT: for all gtsummary tables we need to edit the tables to include specifics  -->
...
<!-- DK: Make residual plots consistent in this chapter! Also, we should ensure that the width of 10 units is the same in each plot. Or does that already happen? Want the intuition that, the more predictive the model, the smaller the remaining variation in the residuals.  -->

<!-- DK: Whenever we have a predictive model, use language like "when comparing." Avoid even the hint of a causal claim. -->
...
<!-- DK: Rewrite along the lines of. The fake data produces some values for `att_end` which are *impossible*. We know from the survey details that the lowest possible value for `att_end` is 3 and the highest is 15. But, in every fake data simulation, we see values less than 3 and greater than 15. This is a flaw in our model. Is it a serious flaw? Tough to know. But, for now, we lack the necessary skills to improve the model enough to remove it. -->

Let's take a deeper look at the graphs that we have above. Our graph in the darker blue represents our actual data. As we can see with the lighter blue graph, our fitted model is able to generate a distribution of the data that is similar when compared to the actual data. One important item that we want to note is that for the actual data there are no values that are above 15. When we compare this to our fitted model we can see that during the "fake-data" simulation there were attitudes that were above 15. 
...
<!-- DK: Answer is predictive. Why? Because it is not causal. Looks for words like "cause" or "affect" or "influence." Look for a question which implies a comparison, *for a single individual unit*, between two states of the world, when where they get treatment X and one where they get treatment Y. Look for a discussion of something which we can *manipulate*. Remember the motto: *No causation without manipulation*. If none of this is present, use a predictive model. If all you need to know to answer the question is the outcome under one value of the treatment, then the model is predictive. Example: What is the att_end for all women if they were to get the treatment? DONE (spread out throughout sections)-->
...
<!-- DK: Add discussion about how the questions, like always, is in exact. Who are we talking about. Let's assume it is all adults in the US in 2012. In fact, the "what are the units" paragraphs always need this discussion, otherwise we can't create a Preceptor Table. And, in addition to units, you need to discuss time. When are we asking this question? Assume it is July 1, 2012. DONE (in the outcomes paragraph as discussed)-->

Third, what are the **units**? Our units for this scenario would be individuals because the questions are about the attributes of unique people at the station.

<!-- DK: Fix this. A "treatment" is just a covariate which, first, we *can* manipulate and, second, we *need* to manipulate, at least in theory, to answer the question. DONE-->
...
<!-- DK: Change this plot to use liberal and income. DONE-->
...
<!-- DK: Sentence should be something about it not being clear how strong the connection is between liberal and income. DONE-->
...
DK: Validity discussions always have one (short) paragraph about each variable, with examples of why validity might not hold. Validity discussion finishes with a brief discussion along the lines of: "Despite these concerns, we will assume that validity does hold."
