...
<!-- MT: do we want posterior predict check in courage or temperance. currently temperance has posterior predict, however, i have added the check before the table because it makes the most sense before putting the data in a table.  -->
...
```{r}
#| echo: false
pp_check(fit_1, plotfun = "hist", nreps = 2, binwidth = 3)
```
...
```{r}
#| echo: false
pp_check(fit_2, plotfun = "scatter", nreps = 3)
```
...
```{r}
#| echo: false
pp_check(fit_4, plotfun = "boxplot", nreps = 3, notch = FALSE)
```
