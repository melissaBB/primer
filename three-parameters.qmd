...
<!-- DK: All Population Tables need a Time columns. (Can use Year, but then mention that it is an example of the moment in time column which we always need.) -->

<!-- DK: Start of each chapter includes all packages, perhaps mentioning why we care. -->

<!-- DK: Do the major surgery we discussed in age ~ party. Put addendum approach as main approach. -->
...
Second, what is the outcome? `age` is the **outcome**. Note that the outcome is not necessarily the same concept mentioned in the questions. For example, the outcome column is not "probablity that a person is over 50" even though that is one of the questions we need to answer.
