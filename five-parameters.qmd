...
The Population Table is a structure which includes a row for every unit in the population. We generally break the rows in the Population Table into three categories: 
1) the data for units we want to have (the actual data set)
2) the data for units which we actually have (the Preceptor Table)
3) the data for units we do not care about (the rest of the population, not included in the data or the Preceptor Table).
...
Representativeness begs the question: is our data representative of the population? *The population is not the set of candidates in our data.* That is the dataset. The population is the larger --- potentially much larger --- set of individuals about whom we want to make inferences. *The parameters in our models refer to the population, not to the dataset.*

Representativeness concerns two relationship:
1) relationship between the Preceptor Table and the other rows;
2) relationship between our data and the other rows.
...
Although it is very rare that the above-mentioned two relationships are representaive of the entire population, we still need to continue our analysis. Therefore, we always assume/pretend that the rows from both the data and the Preceptor Table are representative enough of the larger population.

### Unconfoundedness

Note: unconfoundedness is not a concern for a predictive model. However, this section is targeted to causal model.

Recall that unconfoundedness means that the treatment assignment is independent of the potential outcomes, when we condition on pre-treatment covariates. A model is "confounded” if this is not true.

If the treatment is random, we are mostly confident in its unconfoundedness. The opposite is true for any non-random treatment.

Fortunately, although there might be some issues with confoundedness, they are often small enough to not worry about.
...
**Courage** centers around the creation of the data generating mechanism (DGM), i.e., our fitted model. We often use `stan_glm()` when we want to create a fitted model. With `stan_glm()` we can generate generalized linear or logistic models with optional prior distributions for the coefficients, intercept, and auxiliary parameters.
