...
We do not have a value for female. However we do have an intercept. `sexMale` is the median age of male candidates after winning an election. A higher `MAD_SD` indicates that we are more confident regarding the situation at hand. Typically we don't want to consider variables that are under 2 because they tend to show that we are not confident and don't need to use these variables within the data. However, there are several times where this case may not be accurate, and we will need to use variables that have a `MAD_SD` of under 2.
